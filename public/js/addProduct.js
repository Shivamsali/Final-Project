        ...
        if(file.type.includes('image')){
            ...
            .then(url => {
                fetch(url,{
                    ...
                    let lable = document.querySelector(`label[for=${fileupload.id}]`);
                    lable.style.backgroundImage = `url(${imageUrl})`;
...
const validateForm = () => {
    if(!productName.value.length){
        ...
        return showAlert('short description muct be between 10 to 100 leters long');
...
const setFormData = (data) => {
    ...
    imagePaths.forEach((url, i) => {
        let lable = document.querySelector(`label[for=${uploadImages[i].id}]`);
        lable.style.backgroundImage = `url(${url})`;
        let productImage = document.querySelector('.product-image');
